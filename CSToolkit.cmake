...
set(CFMAKE_EXTERNAL_MKSPECS "" CACHE STRING "Default MKSPECS value for all cfmake_fetch_artifactory and cfmake_fetch_nexus calls if not empty")
...
option(CFMAKE_FETCH_PACKAGE_PREFER_MODULE "If ON, cfmake_fetch_package will search for Find<package> files before <package>Config files" OFF)
option(CFMAKE_FETCH_GIT_SRC "If ON, automatically adds GIT_SRC option to all cfmake_fetch_package calls" OFF)
option(CFMAKE_FETCH_GIT_BUILD "If ON, automatically adds GIT_BUILD option to all cfmake_fetch_package calls" OFF)
option(CFMAKE_FETCH_CHECK_SHA "If ON, redownload file or package if distant SHA has changed" OFF)
...
if(CONAN)
    option(CFMAKE_GENERATE_FETCH_DEPENDENCY "If ON, calls to cfmake_fetch_dependency will be generated in Config files" OFF)
    option(CFMAKE_FETCH_DEPENDENCY "If OFF, calls to cfmake_fetch_dependency will do nothing" OFF)
else()
    option(CFMAKE_GENERATE_FETCH_DEPENDENCY "If ON, calls to cfmake_fetch_dependency will be generated in Config files" ON)
    option(CFMAKE_FETCH_DEPENDENCY "If OFF, calls to cfmake_fetch_dependency will do nothing" ON)
endif()
