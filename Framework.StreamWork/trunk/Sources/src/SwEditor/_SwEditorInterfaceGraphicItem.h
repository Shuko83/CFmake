/*!
 \file _SwEditorInterfaceGraphicItem.h
...
#ifndef __SwEditorInterfaceGraphicItem_H
#define __SwEditorInterfaceGraphicItem_H
...
namespace StreamWork
{
	namespace SwEditor
	{
        /*!
	        ...
	        \brief Class _SwEditorInterfaceGraphicItem gerant la representation graphique d'une interface dans une vue de dessin QT 
		*/
        class _SwEditorInterfaceGraphicItem : public QGraphicsItem, public _ISwEditorGraphicItemListener, public _ISwEditorInterfaceGraphicItemListener{
        protected:
            ...
            QString _type_name;
            /*! \brief Est une interface fournit */ 
            bool _is_provided;
            /*! \brief liste des observers de l'item */
            QSet<_ISwEditorInterfaceGraphicItemListener *> _observers;
            /*! \brief remote item (provider) */
            _SwEditorInterfaceGraphicItem * _remote_item;
            /*! \brief StartPort */
            QRectF _start_port;
            ...
            /*! \brief EndPort position - end port symbol*/
            QPointF _symbol_end_port_position;
            /*! \brief EndPort angle*/
            int _symbol_end_port_angle;
            /*! \brief end port edition*/
            bool _is_end_port_editing;
            /*! \brief end port edition*/
            QRectF _text_area;
            /*! \brief end port edition*/
            ...
            SwCore::ISwInterfaces_Service * _hosting_service;
            /*! \brief liste des positions des points de connexions*/
            QList<QPointF> _connectables_positions;
		public:
            /*! \brief Constructeur */
			_SwEditorInterfaceGraphicItem(_SwEditorGraphicItem * host,QString name,QString type_name,bool is_provided,_SwEditorInterfacesLayerManager * il_manager,SwCore::ISwInterfaces_Service * source);
            /*! \brief Destructeur */
			~_SwEditorInterfaceGraphicItem();
             /*! \brief Definition et acces au type de l'item*/
		    enum { Type = UserType + CG_SW_EDITOR_INTERFACE_GRAPHIC_ITEM_OFFSET_TYPE };
		    ...
		    SwCore::ISwInterfaces_Service * GetInterfaceService();	
		    //-------------------------------------------------------------------
	        ...
	        virtual void OnInterfaceItemChange();            
            //---------------------------------------------------------------------
            // Definition du remote interface item
            //---------------------------------------------------------------------            
            /*! \brief Definition du remote interface item */
            void SetRemoteItem(_SwEditorInterfaceGraphicItem * remote_item);
            /*! \brief Acces au remote interface item */
            _SwEditorInterfaceGraphicItem * GetRemoteItem();
            //---------------------------------------------------------------------
            // Gestion des listeners
            //---------------------------------------------------------------------            
            /*! \brief Ajout d'un listener */
            void Attach_Listener(_ISwEditorInterfaceGraphicItemListener * listener);
            /*! \brief Retrait d'un listener */
            void Detach_Listener(_ISwEditorInterfaceGraphicItemListener * listener);
            /*! \brief Signaler aux listener les changements */
            void SignalChangeToListeners();
            //---------------------------------------------------------------------
            ...
            void OnInterfaceVisibilityChange(const QString * interface_type_name,bool new_visibility);
            /*! \brief Reception de la visibilite d'une inetrface*/
            void OnInterfaceHighlightChange(const QString * interface_type_name,bool set_highlight);
            //---------------------------------------------------------------------
            // Gestion menu
            //---------------------------------------------------------------------
