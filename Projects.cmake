...
function(define_project directory)
...
function(define_component)
  ...
  set(COMPONENT_MULTIPLE PUBLIC_BUILD_DEPS PRIVATE_BUILD_DEPS HEADER_BUILD_DEPS RUNTIME_DEPS)
  cmake_parse_arguments(COMPONENT "${COMPONENT_OPTIONS}" "${COMPONENT_UNIQUE}" "${COMPONENT_MULTIPLE}" ${ARGN})

  list(APPEND ALL_DEPS ${COMPONENT_PUBLIC_BUILD_DEPS} ${COMPONENT_PRIVATE_BUILD_DEPS} ${COMPONENT_HEADER_BUILD_DEPS} ${COMPONENT_RUNTIME_DEPS})
  message("Je print ma liste : ${ALL_DEPS}")
  ...
  foreach(item ${ALL_DEPS})     
    ...
    if(HAS_QT5)         
      ...
      message("Pas de match Regex ${item}")
    endif() 
  endforeach()
  ...
  set(COMPONENT_UI_PATH ${CMAKE_CURRENT_SOURCE_DIR}/..)
  set(COMPONENT_RESOURCES_PATH ${CMAKE_CURRENT_SOURCE_DIR}/..)
  set(COMPONENT_TRANSLATION_PATH ${CMAKE_CURRENT_SOURCE_DIR}/..)

  # Sources

  message(${COMPONENT_UI_PATH})
  ...
  if(NOT COMPONENT_RECURSIVE)
    ...
    file(GLOB COMPONENT_UI_FILES ${COMPONENT_UI_PATH}/*.ui)
    file(GLOB COMPONENT_RESOURCES_FILES ${COMPONENT_RESOURCES_PATH}/*.qrc)
    file(GLOB COMPONENT_TRANSLATION_FILES ${COMPONENT_TRANSLATION_PATH}/*.ts)
    ...
    file(GLOB_RECURSE COMPONENT_UI_FILES ${COMPONENT_UI_PATH}/*.ui)
    file(GLOB_RECURSE COMPONENT_RESOURCES_FILES ${COMPONENT_RESOURCES_PATH}/*.qrc)
    file(GLOB_RECURSE COMPONENT_TRANSLATION_FILES ${COMPONENT_TRANSLATION_PATH}/*.ts)
  ...
  source_group(TREE ${COMPONENT_RESOURCES_PATH} PREFIX "Resource Files" FILES ${COMPONENT_RESOURCES_FILES})
  source_group(TREE ${COMPONENT_TRANSLATION_PATH} PREFIX "Translation Files" FILES ${COMPONENT_TRANSLATION_FILES})
  ...
  if(NOT COMPONENT_EXECUTABLE)
    if(NOT COMPONENT_STATIC AND NOT COMPONENT_INTERFACE)
      add_library(${COMPONENT_NAME} SHARED ${COMPONENT_PUBLIC_HEADERS_FILES} ${COMPONENT_PRIVATE_HEADERS_FILES} ${COMPONENT_SOURCES_FILES} ${COMPONENT_RCFILE_OUT} ${COMPONENT_UI_FILES} ${COMPONENT_RESOURCES_FILES})
  ...
  if(NOT COMPONENT_INTERFACE)
    target_include_directories(${COMPONENT_NAME}
      PUBLIC ${COMPONENT_PUBLIC_HEADERS_PATH}
      PRIVATE ${COMPONENT_PRIVATE_HEADERS_PATH})
  else()
    target_include_directories(${COMPONENT_NAME} INTERFACE
      ${COMPONENT_PUBLIC_HEADERS_PATH}
      ${COMPONENT_PRIVATE_HEADERS_PATH})
  endif()
  ...
  foreach(PUBLIC_BUILD_DEP IN LISTS COMPONENT_PUBLIC_BUILD_DEPS)
    get_target_property(PUBLIC_INTERFACE_INCLUDE_DIRECTORIES ${PUBLIC_BUILD_DEP} INTERFACE_INCLUDE_DIRECTORIES)
    if(PUBLIC_INTERFACE_INCLUDE_DIRECTORIES)
	    target_include_directories(${COMPONENT_NAME} PUBLIC ${PUBLIC_INTERFACE_INCLUDE_DIRECTORIES})
	  endif()
    get_target_property(PUBLIC_INTERFACE_LINK_LIBRARIES ${PUBLIC_BUILD_DEP} INTERFACE_LINK_LIBRARIES)
    if(PUBLIC_INTERFACE_LINK_LIBRARIES)
      target_link_libraries(${COMPONENT_NAME} PUBLIC ${PUBLIC_INTERFACE_LINK_LIBRARIES})
	  endif()
  endforeach()

  foreach(HEADER_BUILD_DEP IN LISTS COMPONENT_HEADER_BUILD_DEPS)
    get_target_property(HEADER_INTERFACE_INCLUDE_DIRECTORIES ${PUBLIC_BUILD_DEP} HEADER_INCLUDE_DIRECTORIES)
    if(HEADER_INTERFACE_INCLUDE_DIRECTORIES)
      target_include_directories(${COMPONENT_NAME} PUBLIC ${PUBLIC_HEADER_INCLUDE_DIRECTORIES})
    endif()
  endforeach()

  foreach(PRIVATE_BUILD_DEP IN LISTS COMPONENT_PRIVATE_BUILD_DEPS)
    get_target_property(PRIVATE_INTERFACE_INCLUDE_DIRECTORIES ${PRIVATE_BUILD_DEP} INTERFACE_INCLUDE_DIRECTORIES)
    if(PRIVATE_INTERFACE_INCLUDE_DIRECTORIES)
      target_include_directories(${COMPONENT_NAME} PRIVATE ${PRIVATE_INTERFACE_INCLUDE_DIRECTORIES})
    endif()
    get_target_property(PRIVATE_INTERFACE_LINK_LIBRARIES ${PRIVATE_BUILD_DEP} INTERFACE_LINK_LIBRARIES)
    if(PRIVATE_INTERFACE_LINK_LIBRARIES)
      target_link_libraries(${COMPONENT_NAME} PRIVATE ${PRIVATE_INTERFACE_LINK_LIBRARIES})
	  endif()
  endforeach()
...
#    endif(WIN32 AND MSVC AND ENABLE_PDB_INSTALL AND NOT COMPONENT_STATIC AND NOT COMPONENT_INTERFACE)
  ...
  generate_target_info(TARGET ${COMPONENT_NAME} COPYRIGHT CSGroup COMPANY CSGroup PRODUCT ${COMPONENT_NAME})
  ...
  message(DEBUG "  - Header build dependencies: ${COMPONENT_HEADER_BUILD_DEPS}")
