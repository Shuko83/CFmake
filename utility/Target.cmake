function(cstoolkit_add_target TARGET_NAME TARGET_TYPE)
    ...
    if(TARGET_EXECUTABLE)
        ...
        set(TARGET_RUNTIME_DLLS "$<LIST:TRANSFORM,${TARGET_RUNTIME_DLLS},REPLACE,(.+),${DOLLAR}<${GENEX_TARGET_EXISTS}:${DOLLAR}<${GENEX_TARGET_DYNAMIC}:${DOLLAR}<TARGET_FILE:\\0$<ANGLE-R>$<ANGLE-R>$<ANGLE-R>>")
        set(TARGET_RUNTIME_DLLS "$<TARGET_GENEX_EVAL:${TARGET_NAME},${TARGET_RUNTIME_DLLS}>")
        ...
        if(TARGET_PLUGINS)
            ...
            while(DESTINATION_INDEX LESS TARGET_PLUGINS_DESTINATION_SIZE)
                ...
                set(PLUGINS_TARGET_FILES${DESTINATION_INDEX} "$<LIST:TRANSFORM,${PLUGINS_TARGET_FILES${DESTINATION_INDEX}},REPLACE,(.+),${DOLLAR}<${GENEX_TARGET_EXISTS}:${DOLLAR}<${GENEX_TARGET_DYNAMIC}:${DOLLAR}<TARGET_FILE:\\0$<ANGLE-R>$<ANGLE-R>$<ANGLE-R>>")
                set(PLUGINS_TARGET_FILES${DESTINATION_INDEX} "$<TARGET_GENEX_EVAL:${TARGET_NAME},${PLUGINS_TARGET_FILES${DESTINATION_INDEX}}>")
    ...
    if(NOT TARGET_NO_INSTALL)
        ...
        if(NOT PROJECT_IS_TOP_LEVEL OR NOT TARGET_NAMESPACE STREQUAL "${PROJECT_NAME}::")
            ...
            if((CSTOOLKIT_INSTALL_TARGETS AND TARGET_NAME IN_LIST CSTOOLKIT_INSTALL_TARGETS) # Specified targets
                    OR NOT CSTOOLKIT_INSTALL_TARGETS) # all targets
...
macro(generate_target_config)
    # Target Config File
    set(DEPENDENCY_GENEX "$<TARGET_PROPERTY:INTERFACE_LINK_LIBRARIES>")
    if(TARGET_STATIC)
        set(DEPENDENCY_GENEX "${DEPENDENCY_GENEX};$<TARGET_PROPERTY:LINK_LIBRARIES>")
    endif()
...
"$<1:$><$<1:$><BOOL:$<1:$><TARGET_PROPERTY:CSTOOLKIT$<COMMA>FETCH_\\1_URL$<ANGLE-R>$<ANGLE-R>:   cstoolkit_fetch_dependency(\\1 \"$<1:$><TARGET_PROPERTY:CSTOOLKIT$<COMMA>FETCH_\\1_URL$<ANGLE-R>\")
$<ANGLE-R>")
