function(cstoolkit_add_target TARGET_NAME TARGET_TYPE)
    ...
    set(TARGET_OPTIONS RECURSIVE RECURSIVE_INCLUDE RECURSIVE_INTERFACE_INCLUDE NO_INSTALL PUBLIC_HEADERS_NO_EXTENSION WIN32 DEPLOY_QT)
    set(TARGET_UNIQUE NAMESPACE ALIAS EXTENSION PLUGINS_DIR FILES_MODE DESCRIPTION)
    set(TARGET_MULTIPLE
        # LIBRARIES
    ...
    # Version
    if(NOT TARGET_EXTENSION AND TARGET_SHARED AND PROJECT_VERSION AND PROJECT_VERSION_MAJOR)
    ...
    elseif(WIN32 AND TARGET_EXECUTABLE AND PROJECT_VERSION)
    ...
    string(REGEX REPLACE "(.*)/${_target_pattern}/" "\\1/" TARGET_FOLDER "${TARGET_FOLDER}")
    ...
    string(REPLACE "/_project/" "/" TARGET_FOLDER "${TARGET_FOLDER}")
    ...
    if(TARGET_SHARED_AND_STATIC)
        set_target_properties(${TARGET_NAME_STATIC} PROPERTIES FOLDER "${TARGET_FOLDER}")
    endif()
    ...
    if(TARGET_EXECUTABLE)
        ...
        set(GENEX_TARGET_SONAME "${DOLLAR}<BOOL:${DOLLAR}<${DOLLAR}<PLATFORM_ID:Linux$<ANGLE-R>:${DOLLAR}<${GENEX_TARGET_SHARED}:${DOLLAR}<BOOL:${DOLLAR}<TARGET_SONAME_FILE_NAME:\\0$<ANGLE-R>$<ANGLE-R>$<ANGLE-R>$<ANGLE-R>$<ANGLE-R>")
        ...
        set(TARGET_RUNTIME_DLLS "$<LIST:TRANSFORM,${TARGET_RUNTIME_DLLS},REPLACE,(.+),${DOLLAR}<${GENEX_TARGET_EXISTS}:${DOLLAR}<${GENEX_TARGET_DYNAMIC}:${DOLLAR}<TARGET_FILE:\\0$<ANGLE-R>$<ANGLE-R>;${DOLLAR}<${GENEX_TARGET_SONAME}:${DOLLAR}<TARGET_SONAME_FILE:\\0$<ANGLE-R>$<ANGLE-R>$<ANGLE-R>>")
        set(TARGET_RUNTIME_DLLS "$<TARGET_GENEX_EVAL:${TARGET_NAME},${TARGET_RUNTIME_DLLS}>")
        ...
        if(TARGET_PLUGINS)
            ...
            while(DESTINATION_INDEX LESS TARGET_PLUGINS_DESTINATION_SIZE)
                ...
                set(PLUGINS_TARGET_FILES${DESTINATION_INDEX} "$<LIST:TRANSFORM,${PLUGINS_TARGET_FILES${DESTINATION_INDEX}},REPLACE,(.+),${DOLLAR}<${GENEX_TARGET_EXISTS}:${DOLLAR}<${GENEX_TARGET_DYNAMIC}:${DOLLAR}<TARGET_FILE:\\0$<ANGLE-R>$<ANGLE-R>;${DOLLAR}<${GENEX_TARGET_SONAME}:${DOLLAR}<TARGET_SONAME_FILE:\\0$<ANGLE-R>$<ANGLE-R>$<ANGLE-R>>")
                set(PLUGINS_TARGET_FILES${DESTINATION_INDEX} "$<TARGET_GENEX_EVAL:${TARGET_NAME},${PLUGINS_TARGET_FILES${DESTINATION_INDEX}}>")
    ...
    if(TARGET_EXECUTABLE)
        ...
        # Qt
        if(_qt_modules AND (CSTOOLKIT_AUTO_DEPLOY_QT OR TARGET_DEPLOY_QT))
