set(USAGE "${CMAKE_COMMAND} -P cfmake_copy.cmake -- [-e] [-d] <file|directory>... <destination_dir>")
...
while(ARGI LESS CMAKE_ARGC)
    ...
    if(IS_OPTION)
        if(CMAKE_ARGV${ARGI} STREQUAL "-d")
            ...
            message(DEBUG "cfmake_copy: DIRECTORY_MODE ON")
        elseif(CMAKE_ARGV${ARGI} STREQUAL "-e")
            set(STOP_ON_ERROR ON)
            message(DEBUG "cfmake_copy: STOP_ON_ERROR ON")
        else()
            message(FATAL_ERROR "cfmake_copy Error: Unknown option \"${CMAKE_ARGV${ARGI}}\"\n"
                                "Usage: ${USAGE}")
...
elseif(NOT IS_DIRECTORY ${DESTINATION})
    message(FATAL_ERROR "cfmake_copy Error: Destination \"${DESTINATION}\" already exists and is not a directory")
    return()
endif()
...
foreach(SOURCE ${SOURCE_LIST})
    if(EXISTS ${SOURCE})
        if(DIRECTORY_MODE AND NOT IS_DIRECTORY ${SOURCE})
            message(FATAL_ERROR "cfmake_copy Error: Source \"${SOURCE}\" is not a directory")
            return()
        elseif(NOT DIRECTORY_MODE AND IS_DIRECTORY ${SOURCE})
            message(FATAL_ERROR "cfmake_copy Error: Source \"${SOURCE}\" is not a file")
            return()
        endif()
        list(APPEND FILTERED_SOURCE_LIST ${SOURCE})
    elseif(STOP_ON_ERROR)
        message(FATAL_ERROR "cfmake_copy Error: Source \"${SOURCE}\" does not exists")
        return()
    endif()
endforeach()
