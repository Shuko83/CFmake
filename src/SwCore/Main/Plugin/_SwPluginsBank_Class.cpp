...
void _SwPluginsBank_Class::addPluginToModel(QString name, TL_plugins& plugins)
{
	...
	else //sinon ajout de la ligne 
	{
		...
		TL_plugins::iterator it=plugins.begin();
		while(it!=plugins.end()) {
			ISwPluginFactory * plugin = *it;
			buildData.clear();
			...
			QSet<QString> setString = plugin->GetComponentsList();
			QSet<QString>::iterator itstring=setString.begin();
			while(itstring!=setString.end()){
			//for (QString compName : plugin->GetComponentsList()) {
				buildData.clear();
				buildData << *itstring << plugin->GetComponentDescription(*itstring);
				plugin_item->appendChild(new _SwTreeItem(buildData, plugin->GetComponentIcon(*itstring), plugin_item));
			}
			it++;
		}
		endInsertRow();
