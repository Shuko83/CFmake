...
void _SwGuiCssHighlighter::highlightBlock(const QString& text)
{
...
void _SwGuiCssHighlighter::highlight(const QString &text, int start, int length, int state)
{
	...
	if(pattern != "")
	{
		...
		myClassFormat.setBackground(QColor("#ffe082"));
		...
		if(isRegx || isMatchWord) // Un peu bourrin
		{
			QString pat = pattern;
		...
		{
			int index = text.indexOf(pattern,0,sensi);
			while(index >=0)
			{
				setFormat(index,pattern.length(),myClassFormat);
				index = text.indexOf(pattern, index + pattern.length(),sensi);
			}
		}
