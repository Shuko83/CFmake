...
_SwGuiCompMainWindow::~_SwGuiCompMainWindow()
{
    ...
    while( dockwidget_it != _dockwidgets.end() )
    {
        if( dockwidget_it.value() )
        {
            ...
            dockwidget_it.value()->setParent( nullptr );
...
void _SwGuiCompMainWindow::initializeComponent() throw( SwException )
{
    ...
    if( _listdockwidgets_nb_property == nullptr )
    {
        if( SW_APP->IsVerbose() )
    ...
    if( _saveAutoPeriod_property == nullptr )
    {
...
void _SwGuiCompMainWindow::interfaceAvailable( QString interfaceName )
{
    ...
    if( listdockwidget_it != _listdockwidgets.end() && listdockwidget_it.value() == nullptr )
    {
    ...
    if( menu_it != _menus.end() && menu_it.value() == nullptr )
    {
    ...
    if( toolbar_it != _toolbars.end() && toolbar_position_it != _toolbar_positions.end() && toolbar_it.value() == nullptr )
    {
...
void _SwGuiCompMainWindow::interfaceUnavailable( QString interfaceName )
{
    ...
    //ListDockWidget
    QMap<QString, ISwListDockWidget *>::iterator listdockwidget_it = _listdockwidgets.find( interfaceName );
    if( listdockwidget_it != _listdockwidgets.end() && listdockwidget_it.value() )
    {
        ...
        listdockwidget_it.value() = nullptr;
        return;
    }
    
    //Menu
    QMap<QString, ISwMenu *>::iterator menu_it = _menus.find( interfaceName );
    if( menu_it != _menus.end() && menu_it.value() )
    {
        menu_it.value()->GetMenu().setParent( nullptr );
        menu_it.value() = nullptr;
        return;
    ...
    //Toolbar
    QMap<QString, ISwToolBar *>::iterator toolbar_it = _toolbars.find( interfaceName );
    if( toolbar_it != _toolbars.end() && toolbar_it.value() )
    {
        toolbar_it.value()->GetToolBar().setParent( nullptr );
        toolbar_it.value() = nullptr;
        return;
