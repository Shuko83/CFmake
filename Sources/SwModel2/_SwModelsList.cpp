...
void _SwModelsList::LoadModels() {
    ...
    //Test ressources
    QFile resourceList(QString(":/SwModel2/")+CL_MODELS_LIST);
    if (filename.isEmpty() && resourceList.exists()) {
        filename=QString(":/SwModel2/")+CL_MODELS_LIST;
        dir=QDir(QString(":/SwModel2/")+CL_MODELS_DIRECTORY);
    }
    //Sinon
    if(filename.isEmpty()) {
...
bool _SwModelsList::checkModelName(QString modelName)
{
	...
	for ( QDomElement model_elt = root_elt.firstChildElement(CL_MODEL_NODE); !model_elt.isNull(); model_elt = model_elt.nextSiblingElement(CL_MODEL_NODE) )
	{
		...
		if ( model_elt.hasAttribute(CL_MODEL_NAME) &&
			...
			model_elt.hasAttribute(CL_MODEL_IPATH) )
		{
			if ( model_elt.attribute(CL_MODEL_NAME) == modelName )
				return true;
		}
	}
