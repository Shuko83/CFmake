...
void QtCursorDatabase::clear()
{
    _cursorNames.clear();
    _cursorIcons.clear();
    _valueToCursorShape.clear();
    _cursorShapeToValue.clear();
}

void QtCursorDatabase::appendCursor( Qt::CursorShape shape, const QString & name, const QIcon & icon )
{
    if( _cursorShapeToValue.contains( shape ) )
        return;
    const int value = _cursorNames.count();
    _cursorNames.append( name );
    _cursorIcons.insert( value, icon );
    _valueToCursorShape.insert( value, shape );
    _cursorShapeToValue.insert( shape, value );
}
...
int QtCursorDatabase::cursorToValue( const QCursor & cursor ) const
{
    ...
    if( _cursorShapeToValue.contains( shape ) )
        return _cursorShapeToValue[shape];
    #endif
    return -1;
}
...
{
    if( _valueToCursorShape.contains( value ) )
        return QCursor( _valueToCursorShape[value] );
    return QCursor();
}
#endif
...
{
    return _checkBox->blockSignals( block );
}
