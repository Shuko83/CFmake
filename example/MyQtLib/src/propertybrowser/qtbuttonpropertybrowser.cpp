...
private:
    ...
    QMap< QtBrowserItem *, WidgetItem * > _indexToItem;
    QMap< WidgetItem *, QtBrowserItem * > _itemToIndex;
    QMap< QWidget *, WidgetItem * > _widgetToItem;
    QMap< QObject *, WidgetItem * > _buttonToItem;
    QGridLayout * _mainLayout;
    QList< WidgetItem * > _children;
    QList< WidgetItem * > _recreateQueue;
};
...
void QtButtonPropertyBrowserPrivate::init( QWidget * parent )
{
    ...
    parent->setLayout( _mainLayout );
    QLayoutItem * item = new QSpacerItem( 0, 0,
                                          QSizePolicy::Fixed, QSizePolicy::Expanding );
    _mainLayout->addItem( item, 0, 0 );
}
...
void QtButtonPropertyBrowserPrivate::slotEditorDestroyed()
{
    ...
    if( !editor )
        return;
    if( !_widgetToItem.contains( editor ) )
        return;
    _widgetToItem[editor]->widget = 0;
    _widgetToItem.remove( editor );
}

void QtButtonPropertyBrowserPrivate::slotUpdate()
{
    QListIterator< WidgetItem * > itItem( _recreateQueue );
...
{
    WidgetItem * item = _buttonToItem.value( q_ptr->sender() );
    if( !item )
        return;
    ...
    if( checked )
        emit q_ptr->expanded( _itemToIndex.value( item ) );
    else
        emit q_ptr->collapsed( _itemToIndex.value( item ) );
}
...
{
    WidgetItem * afterItem = _indexToItem.value( afterIndex );
    WidgetItem * parentItem = _indexToItem.value( index->parent() );
    ...
    else
    {
        ...
        if( parentItem )
            ...
            _children.insert( _children.indexOf( afterItem ) + 1, newItem );
    }
    ...
    else
    {
        ...
        {
            _recreateQueue.removeAll( parentItem );
            ...
            _buttonToItem[parentItem->button] = parentItem;
    ...
    if( newItem->widget )
    {
        ...
        _widgetToItem[newItem->widget] = newItem;
    }
...
{
    WidgetItem * item = _indexToItem.value( index );
    ...
    _buttonToItem.remove( item->button );
    ...
    else
    {
        ...
        parentItem->layout = 0;
        if( !_recreateQueue.contains( parentItem ) )
            _recreateQueue.append( parentItem );
        if( parentSpan > 1 )
            removeRow( l, parentRow + 1 );

        updateLater();
    }
    _recreateQueue.removeAll( item );
...
{
    const QMap< QtButtonPropertyBrowserPrivate::WidgetItem *, QtBrowserItem * >::ConstIterator icend = d_ptr->_itemToIndex.constEnd();
    for( QMap< QtButtonPropertyBrowserPrivate::WidgetItem *, QtBrowserItem * >::ConstIterator  it =  d_ptr->_itemToIndex.constBegin(); it != icend; ++it )
        delete it.key();
}
...
{
    QtButtonPropertyBrowserPrivate::WidgetItem * itm = d_ptr->_indexToItem.value( item );
    if( itm )
