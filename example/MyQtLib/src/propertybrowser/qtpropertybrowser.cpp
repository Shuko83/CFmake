...
{
public:
    QtPropertyPrivate( QtAbstractPropertyManager * manager ) : _enabled( true ), _modified( false ), _manager( manager ) {}
    QtProperty * q_ptr;
    
    QSet< QtProperty * > _parentItems;
    QList< QtProperty * > _subItems;
    ...
    QtAbstractPropertyManager * const _manager;
};
...
{
    QSetIterator< QtProperty * > itParent( d_ptr->_parentItems );
    while( itParent.hasNext() )
    {
        QtProperty * property = itParent.next();
        property->d_ptr->_manager->d_ptr->propertyRemoved( this, property );
    }
    
    d_ptr->_manager->d_ptr->propertyDestroyed( this );
    
    QListIterator< QtProperty * > itChild( d_ptr->_subItems );
    while( itChild.hasNext() )
    {
        QtProperty * property = itChild.next();
        property->d_ptr->_parentItems.remove( this );
    }
    ...
    {
        QtProperty * property = itParent.next();
        property->d_ptr->_subItems.removeAll( this );
    }
}
...
{
    return d_ptr->_manager->hasValue( this );
}
...
{
    return d_ptr->_manager->valueIcon( this );
}
...
{
    return d_ptr->_manager->valueText( this );
}
...
{
    if( d_ptr->_statusTip == text )
        return;
        
    d_ptr->_statusTip = text;
    propertyChanged();
}
...
{
    if( d_ptr->_whatsThis == text )
        return;
        
    d_ptr->_whatsThis = text;
    propertyChanged();
}
...
{
    if( d_ptr->_enabled == enable )
        return;
        
    d_ptr->_enabled = enable;
    propertyChanged();
}
...
{
    if( d_ptr->_modified == modified )
        return;
        
    d_ptr->_modified = modified;
    propertyChanged();
}
...
{
    QtProperty * after = 0;
    if( d_ptr->_subItems.count() > 0 )
        after = d_ptr->_subItems.last();
                                    ...
                                    QtProperty * afterProperty )
{
    ...
    d_ptr->_subItems.insert( newPos, property );
    property->d_ptr->_parentItems.insert( this );
    
    d_ptr->_manager->d_ptr->propertyInserted( property, this, properAfterProperty );
}
...
void QtProperty::removeSubProperty( QtProperty * property )
{
    ...
    d_ptr->_manager->d_ptr->propertyRemoved( property, this );
...
{
    d_ptr->_manager->d_ptr->propertyChanged( this );
}
...
QtProperty * QtAbstractPropertyManager::addProperty( const QString & name )
{
    ...
    if( property )
    {
        ...
        d_ptr->_properties.insert( property );
...
public:
    QtBrowserItemPrivate( QtAbstractPropertyBrowser * browser, QtProperty * property, QtBrowserItem * parent )
        : _browser( browser ), _property( property ), _parent( parent ), q_ptr( 0 ) {}
        
    void addChild( QtBrowserItem * index, QtBrowserItem * after );
    void removeChild( QtBrowserItem * index );
    
    QtAbstractPropertyBrowser * const _browser;
    QtProperty * _property;
    QtBrowserItem * _parent;
    
    QtBrowserItem * q_ptr;
    
    QList< QtBrowserItem * > _children;
...
void QtBrowserItemPrivate::addChild( QtBrowserItem * index, QtBrowserItem * after )
{
    if( _children.contains( index ) )
        return;
    int idx = _children.indexOf( after ) + 1; // we insert after returned idx, if it was -1 then we set idx to 0;
    _children.insert( idx, index );
}

void QtBrowserItemPrivate::removeChild( QtBrowserItem * index )
{
    _children.removeAll( index );
}
...
Q_GLOBAL_STATIC( Map1, _viewToManagerToFactory )
Q_GLOBAL_STATIC( Map2, _managerToFactoryToViews )
...
public:
    ...
    QList< QtProperty * > _subItems;
    QMap< QtAbstractPropertyManager *, QList< QtProperty * > > _managerToProperties;
    QMap< QtProperty *, QList< QtProperty * > > _propertyToParents;
    
    QMap< QtProperty *, QtBrowserItem * > _topLevelPropertyToIndex;
    QList< QtBrowserItem * > _topLevelIndexes;
    QMap< QtProperty *, QList< QtBrowserItem * > > _propertyToIndexes;
    
    QtBrowserItem * _currentItem;
};
...
void QtAbstractPropertyBrowserPrivate::insertSubTree( QtProperty * property,
        QtProperty * parentProperty )
{
    if( _propertyToParents.contains( property ) )
    {
        ...
        _propertyToParents[property].append( parentProperty );
        // don't need to update _managerToProperties map since
        // _managerToProperties[manager] already contains property.
        return;
    }
    QtAbstractPropertyManager * manager = property->propertyManager();
    if( _managerToProperties[manager].isEmpty() )
    {
    ...
    _managerToProperties[manager].append( property );
    _propertyToParents[property].append( parentProperty );
        ...
        QtProperty * parentProperty )
{
    ...
    _propertyToParents[property].removeAll( parentProperty );
    if( !_propertyToParents[property].isEmpty() )
        return;
        
    _propertyToParents.remove( property );
    QtAbstractPropertyManager * manager = property->propertyManager();
    _managerToProperties[manager].removeAll( property );
    if( _managerToProperties[manager].isEmpty() )
    {
        ...
        _managerToProperties.remove( manager );
    }
...
void QtAbstractPropertyBrowserPrivate::createBrowserIndexes( QtProperty * property, QtProperty * parentProperty, QtProperty * afterProperty )
{
    ...
    if( afterProperty )
    {
        QMap< QtProperty *, QList< QtBrowserItem * > >::ConstIterator it =
            _propertyToIndexes.find( afterProperty );
        if( it == _propertyToIndexes.constEnd() )
            return;
    ...
    {
        QMap< QtProperty *, QList< QtBrowserItem * > >::ConstIterator it =
            _propertyToIndexes.find( parentProperty );
        if( it == _propertyToIndexes.constEnd() )
            return;
        ...
        QtBrowserItem * parentIndex, QtBrowserItem * afterIndex )
{
    ...
    {
        _topLevelPropertyToIndex[property] = newIndex;
        _topLevelIndexes.insert( _topLevelIndexes.indexOf( afterIndex ) + 1, newIndex );
    }
    _propertyToIndexes[property].append( newIndex );
...
void QtAbstractPropertyBrowserPrivate::removeBrowserIndexes( QtProperty * property, QtProperty * parentProperty )
{
    ...
    QMap< QtProperty *, QList< QtBrowserItem * > >::ConstIterator it =
        _propertyToIndexes.find( property );
    if( it == _propertyToIndexes.constEnd() )
        return;
...
void QtAbstractPropertyBrowserPrivate::removeBrowserIndex( QtBrowserItem * index )
{
    ...
    {
        _topLevelPropertyToIndex.remove( index->property() );
        _topLevelIndexes.removeAll( index );
    }
    
    QtProperty * property = index->property();
    
    _propertyToIndexes[property].removeAll( index );
    if( _propertyToIndexes[property].isEmpty() )
        _propertyToIndexes.remove( property );
...
void QtAbstractPropertyBrowserPrivate::slotPropertyInserted( QtProperty * property,
        QtProperty * parentProperty, QtProperty * afterProperty )
{
    if( !_propertyToParents.contains( parentProperty ) )
        return;
...
{
    if( !_subItems.contains( property ) )
        return;
...
{
    return d_ptr->_propertyToIndexes.value( property );
}
...
{
    return d_ptr->_topLevelPropertyToIndex.value( property );
}
...
QtBrowserItem * QtAbstractPropertyBrowser::addProperty( QtProperty * property )
{
    ...
    if( d_ptr->_subItems.count() > 0 )
        afterProperty = d_ptr->_subItems.last();
...
void QtAbstractPropertyBrowser::removeProperty( QtProperty * property )
{
    ...
    while( pos < pendingList.count() )
    {
        if( pendingList.at( pos ) == property )
        {
            d_ptr->_subItems.removeAt( pos ); //perhaps this two lines
        ...
        QWidget * parent )
{
    ...
    if( _viewToManagerToFactory()->contains( this ) &&
        ( *_viewToManagerToFactory() )[this].contains( manager ) )
        factory = ( *_viewToManagerToFactory() )[this][manager];
        ...
        QtAbstractEditorFactoryBase * abstractFactory )
{
    ...
    if( !_managerToFactoryToViews()->contains( abstractManager ) ||
        !( *_managerToFactoryToViews() )[abstractManager].contains( abstractFactory ) )
        connectNeeded = true;
    else if( ( *_managerToFactoryToViews() )[abstractManager][abstractFactory]
             .contains( this ) )
        return connectNeeded;
        
    if( _viewToManagerToFactory()->contains( this ) &&
        ( *_viewToManagerToFactory() )[this].contains( abstractManager ) )
        unsetFactoryForManager( abstractManager );
        
    ( *_managerToFactoryToViews() )[abstractManager][abstractFactory].append( this );
    ( *_viewToManagerToFactory() )[this][abstractManager] = abstractFactory;
...
void QtAbstractPropertyBrowser::unsetFactoryForManager( QtAbstractPropertyManager * manager )
{
    ...
    QtAbstractEditorFactoryBase * abstractFactory =
        ( *_viewToManagerToFactory() )[this][manager];
    ( *_viewToManagerToFactory() )[this].remove( manager );
    if( ( *_viewToManagerToFactory() )[this].isEmpty() )
        ( *_viewToManagerToFactory() ).remove( this );
        
    ( *_managerToFactoryToViews() )[manager][abstractFactory].removeAll( this );
    if( ( *_managerToFactoryToViews() )[manager][abstractFactory].isEmpty() )
    {
        ( *_managerToFactoryToViews() )[manager].remove( abstractFactory );
        abstractFactory->breakConnection( manager );
        if( ( *_managerToFactoryToViews() )[manager].isEmpty() )
            ( *_managerToFactoryToViews() ).remove( manager );
    }
}
...
{
    QtBrowserItem * oldItem = d_ptr->_currentItem;
    d_ptr->_currentItem = item;
    if( oldItem != item )
