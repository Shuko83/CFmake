...
void QtGroupBoxPropertyBrowserPrivate::propertyInserted( QtBrowserItem * index, QtBrowserItem * afterIndex )
{
    ...
    else
    {
        ...
        {
            row = _children.indexOf( afterItem ) + 1;
            _children.insert( row, newItem );
        }
    }
    ...
    else
    {
        if( !parentItem->groupBox )
        {
            ...
            if( !par )
            {
                w = q_ptr;
                l = _mainLayout;
                oldRow = _children.indexOf( parentItem );
            }
            else
            {
                w = par->groupBox;
...
{
    const QMap< QtGroupBoxPropertyBrowserPrivate::WidgetItem *, QtBrowserItem * >::ConstIterator icend = d_ptr->_itemToIndex.constEnd();
    for( QMap< QtGroupBoxPropertyBrowserPrivate::WidgetItem *, QtBrowserItem * >::ConstIterator it = d_ptr->_itemToIndex.constBegin(); it != icend; ++it )
        delete it.key();
}
