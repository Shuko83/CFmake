    ...
    for(int i=0;i<_entities_list->count();i++) {
        switch((*_entities_list)[i]->_type) {
            case Ent_Property:
                ...
                break;
            case Ent_InterfaceC:
                _interfacesC.push_back(new _Item(IT_Interface_C,(*_entities_list)[i]->_name,(*_entities_list)[i]->_exported_name,(*_entities_list)[i]->_host_path));
                break;
            case Ent_InterfaceP:
                _interfacesP.push_back(new _Item(IT_Interface_P,(*_entities_list)[i]->_name,(*_entities_list)[i]->_exported_name,(*_entities_list)[i]->_host_path));
                break;
            case Ent_Pin:
                _pins.push_back(new _Item(IT_Pin,(*_entities_list)[i]->_name,(*_entities_list)[i]->_exported_name,(*_entities_list)[i]->_host_path));
                break;
            case Ent_Executable:
            	_executables.push_back(new _Item(IT_Executable,(*_entities_list)[i]->_name,(*_entities_list)[i]->_exported_name,(*_entities_list)[i]->_host_path));
                break;
...
bool _SwModelExportedEntitiesModel::dropMimeData ( const QMimeData * data, Qt::DropAction action, int row, int column, const QModelIndex & parent ) {
    if (data->hasFormat("application/model_entity")) {
        ...
        for(int i=0;i<newItems.count();i++) {
            ...
            if (type==QString("IC")) {
                _interfacesC.push_back(new _Item(IT_Interface_C,name,exported_name,item_parts[0]));
            }
            if (type==QString("IP")) {
                _interfacesP.push_back(new _Item(IT_Interface_P,name,exported_name,item_parts[0]));
            }
            if (type==QString("PI")) {
                _pins.push_back(new _Item(IT_Pin,name,exported_name,item_parts[0]));
            }
            if (type==QString("EX") && _executables.count()==0) {
                _executables.push_back(new _Item(IT_Executable,"Executable interface","Executable interface",item_parts[0]));
            }
            if (type==QString("CO") && _ownerConfigurables.count()==0) {
                _ownerConfigurables.push_back(new _Item(IT_OwnerConfigurable,"OwnerConfigurable interface","OwnerConfigurable interface",item_parts[0]));
            }            
        }
        emit layoutChanged();
        return true;
